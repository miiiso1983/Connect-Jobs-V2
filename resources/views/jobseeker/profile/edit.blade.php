<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-base-content leading-tight">بروفايل المتقدم</h2>
    </x-slot>

    <div class="py-8 max-w-4xl mx-auto sm:px-6 lg:px-8">
        @if (session('status'))
            <div class="alert alert-success shadow mb-4">{{ session('status') }}</div>
        @endif
        <form method="POST" action="{{ route('jobseeker.profile.update') }}" enctype="multipart/form-data" class="card bg-base-100 p-6 shadow grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            ...
            <div x-data="districtPicker()" x-init="init('{{ old('province', $js->province ?? '') }}', @js(old('districts', $js->districts ?? [])))">
                ...
                <div class="mt-3">
                    ...
                    <div class="mt-2 max-h-48 overflow-y-auto rounded border border-base-300 p-3 space-y-2">
                        <template x-for="d in districts" :key="d">
                            <label class="flex items-center gap-2">
                                <input type="checkbox" :value="d" name="districts[]" x-model="selected" class="checkbox checkbox-primary">
                                <span x-text="d" class="text-sm text-base-content/70"></span>
                            </label>
                        </template>
                        <template x-if="!districts.length">
                            <div class="text-sm text-base-content/60">اختر محافظة لعرض المناطق</div>
                        </template>
                    </div>
            ...
            <div x-data="{selected: @js(old('specialities', $js->specialities ?? [])), options: @js($specialities->sort()->values())}">
                ...
                <div class="mt-2 max-h-48 overflow-y-auto rounded border border-base-300 p-3 space-y-2">
                    <template x-for="s in options" :key="s">
                        <label class="flex items-center gap-2">
                            <input type="checkbox" :value="s" name="specialities[]" x-model="selected" class="checkbox checkbox-primary">
            ...
            <div>
                ...
                <input list="specialities" id="speciality" name="speciality" class="input input-bordered w-full mt-1" value="{{ old('speciality', $js->speciality ?? '') }}" />
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="summary" name="summary" rows="3" class="textarea textarea-bordered w-full mt-1">{{ old('summary', $js->summary ?? '') }}</textarea>
            </div>
            <div class="md:col-span-2">
                <x-input-label for="qualifications" value="المؤهلات" />
                <textarea id="qualifications" name="qualifications" rows="3" class="textarea textarea-bordered w-full mt-1">{{ old('qualifications', $js->qualifications ?? '') }}</textarea>
            </div>
            <div class="md:col-span-2">
                <x-input-label for="experiences" value="الخبرات" />
                <textarea id="experiences" name="experiences" rows="3" class="textarea textarea-bordered w-full mt-1">{{ old('experiences', $js->experiences ?? '') }}</textarea>
            </div>
            <div class="md:col-span-2">
                <x-input-label for="languages" value="اللغات" />
                <textarea id="languages" name="languages" rows="2" class="textarea textarea-bordered w-full mt-1">{{ old('languages', $js->languages ?? '') }}</textarea>
            </div>
            <div class="md:col-span-2">
                <x-input-label for="skills" value="المهارات" />
                <textarea id="skills" name="skills" rows="2" class="textarea textarea-bordered w-full mt-1" placeholder="مثال: excel, sales, pediatrics">{{ old('skills', $js->skills ?? '') }}</textarea>
            </div>
@push('scripts')
<script>
...
@endpush
            <div class="md:col-span-2">
                ...
                <input id="cv" name="cv" type="file" class="file-input file-input-bordered w-full mt-1" accept=".pdf,.doc,.docx" />
                @if(!empty($js->cv_file))
