    ...
    <div class="py-8 max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <form method="POST" action="{{ route('admin.settings.store') }}" class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow flex gap-3 items-end">
            ...
            <div class="flex-1">
                ...
                <x-text-input id="value" name="value" class="block mt-1 w-full" required />
            </div>
            <x-primary-button>إضافة</x-primary-button>
        </form>

        {{-- إضافة جماعية --}}
        <form method="POST" action="{{ route('admin.settings.bulk') }}" class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow flex flex-col gap-2">
            @csrf
            <div class="text-sm font-semibold text-gray-700 dark:text-gray-200">إضافة عدة قيم دفعة واحدة (قيمة في كل سطر)</div>
            <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600 dark:text-gray-400">النوع:</label>
                <select name="setting_type" class="rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700">
                    @foreach (['job_title','province','speciality','gender','education_level','experience_level'] as $t)
                        <option value="{{ $t }}">{{ $t }}</option>
                    @endforeach
                </select>
        ...
        {{-- تصدير CSV --}}
        <form method="GET" action="{{ route('admin.settings.export') }}" class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow flex items-end gap-3">
            <div>
        ...
        {{-- استيراد CSV --}}
        <form method="POST" action="{{ route('admin.settings.import') }}" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow flex items-end gap-3">
            @csrf
            <div class="flex-1">
