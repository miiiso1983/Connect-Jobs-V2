    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-[#0D2660] via-[#102E66] to-[#0A1E46] text-white py-12">
        ...
        <div class="relative max-w-7xl mx-auto px-4 text-center">
            <div class="flex items-center justify-center gap-3 mb-4">
                <div class="w-12 h-12 rounded-xl bg-[#E7C66A]/20 flex items-center justify-center">
                    <svg class="w-6 h-6 text-[#E7C66A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                </div>
            </div>
            <h1 class="text-3xl md:text-4xl font-extrabold mb-2">الوظائف المتاحة</h1>
            <p class="text-[#E7C66A] text-lg">اكتشف أفضل الفرص الوظيفية المناسبة لمهاراتك</p>
            @if(count($titleParts))
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden mb-8">
                ...
                <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-3">
                        <button type="submit" class="px-6 py-2.5 rounded-lg bg-gradient-to-r from-[#0D2660] to-[#102E66] hover:from-[#0A1E46] hover:to-[#0D2660] text-white font-bold transition-all duration-300 shadow-lg hover:shadow-xl flex items-center gap-2">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                <div class="flex items-center gap-3 p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <span class="bg-[#0D2660] text-white text-xs font-bold px-3 py-1 rounded-full">{{ $jobs->total() }} وظيفة</span>
                </div>
                <div class="p-6">
                            ...
                            <div class="group bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-750 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 relative overflow-hidden">
                                ...
                                <div class="flex items-start gap-3">
                                    @if($job->company && !empty($job->company->profile_image))
                                        <img src="{{ Storage::url($job->company->profile_image) }}" alt="{{ $job->company->company_name }}" class="w-12 h-12 rounded-lg object-cover ring-2 ring-[#0D2660]/20"/>
                                    @else
                                        <div class="w-12 h-12 rounded-lg bg-gradient-to-br from-[#0D2660] to-[#102E66] flex items-center justify-center">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ route('jobs.show',$job) }}" class="block text-base font-bold text-[#0D2660] dark:text-[#E7C66A] hover:underline truncate">{{ $job->title }}</a>
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            @if($job->company)
                                                <a href="{{ route('public.company.show',$job->company) }}" class="hover:text-[#0D2660] dark:hover:text-[#E7C66A]">{{ $job->company->company_name }}</a>
                                            @else
                                                —
                                            @endif
                                        </div>
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    ...
                                    @if(optional($job->company)->industry)
                                        <span class="px-2 py-1 rounded-full text-xs bg-[#0D2660]/10 dark:bg-[#0D2660]/30 text-[#0D2660] dark:text-[#E7C66A]">{{ optional($job->company)->industry }}</span>
                                    @endif
                                </div>
                                    ...
                                    @auth
                                        @if(auth()->user()->role==='jobseeker')
                                            ...
                                            <form method="POST" action="{{ $isSaved ? route('jobs.unsave',$job) : route('jobs.save',$job) }}">
                                                ...
                                                <button class="text-xs text-[#0D2660] dark:text-[#E7C66A] hover:underline font-medium flex items-center gap-1" type="submit">
