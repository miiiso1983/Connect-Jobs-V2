  ...
  {{-- Hero Header --}}
    ...
    <div class="relative max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center gap-6">
      @if(!empty($company->profile_image))
        ...
        @endphp
	        <img src="{{ Storage::url($company->profile_image) }}" @if($srcset) srcset="{{ $srcset }}" sizes="96px" @endif alt="{{ $company->company_name }}" loading="lazy" width="96" height="96" class="w-24 h-24 rounded-2xl object-cover ring-4 ring-[#38BDF8]/30 shadow-xl"/>
      @else
	      <div class="w-24 h-24 rounded-2xl bg-[#38BDF8]/20 flex items-center justify-center">
	        <svg class="w-12 h-12 text-[#38BDF8]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
        </div>
      @endif
      <div class="text-center md:text-right">
        ...
        <div class="flex flex-wrap justify-center md:justify-start gap-3 mt-3">
          ...
          @if($company->province)
            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-sm">
	              <svg class="w-4 h-4 text-[#38BDF8]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
              {{ $company->province }}
            </span>
          @endif
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
          <div class="flex items-center gap-3 p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <h3 class="text-lg font-bold text-gray-800 dark:text-white">الوظائف المفتوحة</h3>
	            <span class="bg-[#5B21B6] text-white text-xs font-bold px-3 py-1 rounded-full">{{ $company->jobs->count() }} وظيفة</span>
          </div>
            ...
            @forelse($company->jobs as $job)
              <div class="group bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-750 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-start justify-between gap-4">
                  <div class="flex-1">
	                    <a href="{{ route('jobs.show',$job) }}" class="text-lg font-bold text-[#5B21B6] dark:text-[#38BDF8] hover:underline">{{ $job->title }}</a>
                    ...
                    <p class="mt-2 text-gray-600 dark:text-gray-300 line-clamp-2">{{ \Illuminate\Support\Str::limit(strip_tags((string)$job->description), 140) }}</p>
	                  ...
	                  <a href="{{ route('jobs.show',$job) }}" class="flex-shrink-0 px-4 py-2 rounded-lg bg-gradient-to-r from-[#5B21B6] to-[#6D28D9] text-white text-sm font-medium hover:from-[#4C1D95] hover:to-[#5B21B6] transition-all shadow">
                    عرض التفاصيل
                  </a>
                </div>
              </div>
        ...
        {{-- Browse Jobs CTA --}}
	        <div class="bg-gradient-to-br from-[#5B21B6] to-[#4C1D95] rounded-xl shadow-lg p-6 text-center text-white">
	          <div class="w-14 h-14 mx-auto mb-4 rounded-full bg-[#38BDF8]/20 flex items-center justify-center">
	            <svg class="w-7 h-7 text-[#38BDF8]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          </div>
          <h4 class="font-bold text-lg mb-2">تصفح المزيد</h4>
          <p class="text-white/80 text-sm mb-4">اكتشف المزيد من الفرص الوظيفية</p>
	          <a href="{{ route('jobs.index') }}" class="block w-full py-3 rounded-lg bg-[#38BDF8] text-[#4C1D95] font-bold hover:bg-[#7DD3FC] transition">
            تصفح جميع الوظائف
          </a>
        </div>
      </aside>
